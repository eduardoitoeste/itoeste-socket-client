...
export default class Connection extends Emitter {
  constructor (url, debug, options) {
    ...
    this.debugOpt = debug
    console.log('INIT',this.debugOpt)
    if (this.debugOpt) {
  ...
  _ensureSubscription (packet, cb) {
    ...
    if (!socket) {
      if (this.debugOpt) {
        console.error('cannot consume packet since %s topic has no active subscription %j', packet.d.topic, packet)
      }
      return
    }
  ...
  _processQueue () {
    ...
    this.options.encoder.encode(this._packetsQueue.shift(), (error, payload) => {
      if (error) {
        if (this.debugOpt) {
          console.error('encode error %j', error)
        }
        return
      }
      this.write(payload)
  ...
  _onMessage (event) {
    this.options.encoder.decode(event.data, (decodeError, packet) => {
      if (decodeError) {
        if (this.debugOpt) {
          console.error('packet dropped, decode error %o', decodeError)
        }
        return
      }
  ...
  _handleMessage (packet) {
    ...
    if (wsp.isJoinErrorPacket(packet)) {
      if (this.debugOpt) {
        console.error('join error packet')
      }
    ...
    if (wsp.isLeaveErrorPacket(packet)) {
      if (this.debugOpt) {
        console.error('leave error packet')
      }
    ...
    if (this.debugOpt) {
      console.error('invalid packet type %d', packet.t)
    }
  }
